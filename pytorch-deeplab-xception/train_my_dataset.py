...
from dataloaders.datasets.crack_images import CrackImagesForSegmentation
...
class Trainer(object):
    def __init__(self, args):
        ...
        # Define network
        model = DeepLab(num_classes=dataset_train.NUM_CLASSES,
        ...
        if args.use_balanced_weights:
            ...
            if os.path.isfile(classes_weights_path):
                ...
                weight = calculate_weigths_labels(
                    args.dataset, self.train_loader, dataset_train.NUM_CLASSES)
            weight = torch.from_numpy(weight.astype(np.float32))
        else:
            weight = None
        self.criterion = SegmentationLosses(
            weight=weight, cuda=args.cuda, device=DEVICE).build_loss(mode=args.loss_type)
        self.model, self.optimizer = model, optimizer

        # Define Evaluator
        self.evaluator = Evaluator(dataset_train.NUM_CLASSES)
        # Define lr scheduler
